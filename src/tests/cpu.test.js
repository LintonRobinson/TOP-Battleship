...
describe("Cpu", () => {
  ...
  describe("Cpu.getCoordinateToAttackPlayerGameboard()", () => {
    ...
    describe("when there are cells in coordinatesToExploreFromHitShip", () => {
      testCpu.cellsToExploreFromHitShip = [
        {
          cellCoordinate: "D5",
      ...
      it("returns a coordinate that is not in missedCells or hitCells", () => {
        expect(testCpu.hitCells.has(testCoordinate.cellCoordinate)).toBe(false);
        expect(testCpu.missedCells.has(testCoordinate.cellCoordinate)).toBe(false);
      });

      it("returns an object with the property cellCoordinate  with its value being the cell object of cellsToExploreFromHitShip at the index of cellsToExploreFromHitShipQueueIndexPosition", () => {
        expect(testCoordinate).toHaveProperty("cell", {
          cellCoordinate: "D5",
          relativeCellOrientationOrigin: "top",
        });
      });

      it("returns an object with the property cellCoordinateOrigin with its value being 'cellsToExploreFromHitShip'", () => {
        expect(testCoordinate).toHaveProperty("cellCoordinateOrigin", "cellsToExploreFromHitShip");
      });
    });
  });
  ...
  describe("Cpu.generateFutureShot()", () => {
    it("returns the correct relative cell coordinate", () => {
      ...
      expect(testCpu.generateFutureShot("C3", "left")).toHaveProperty("cellCoordinate", "B3");
      expect(testCpu.generateFutureShot("C3", "right")).toHaveProperty("cellCoordinate", "D3");
  ...
  describe("Cpu.removeCellFromCellsToExploreFromHitShip()", () => {
    it("removes the correct cell coordinate from cellsToExploreFromHitShip", () => {
      testCpu.cellsToExploreFromHitShip = [
        { cellCoordinate: "A1", relativeCellOrientationOrigin: "top" },
        { cellCoordinate: "D5", relativeCellOrientationOrigin: "bottom" },
        { cellCoordinate: "J2", relativeCellOrientationOrigin: "left" },
      ];

      testCpu.removeCellFromCellsToExploreFromHitShip("D5");
      expect(testCpu.cellsToExploreFromHitShip.some((cell) => cell.cellCoordinate === "D5")).toBe(false);
    });
  });
});
